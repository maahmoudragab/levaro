...
import type { ProductItem, DepartmentKey, CategoryKey, CollectionKey } from "@/types/storefront";
...
function mapSupabaseRowToStorefrontProduct(
  ...
  return {
    ...
    name,
    department: normalizeDepartment(row.gender as string | null, categorySlug),
  ...
  async (): Promise<ProductItem[]> => {
    try {
      ...
      return data.map((row, index) =>
        mapSupabaseRowToStorefrontProduct(supabase, row as Record<string, unknown>, index)
      );
    } catch (err) {
...
export const getStorefrontProductBySlug = unstable_cache(
  async (slug: string): Promise<ProductItem | null> => {
    try {
      ...
      return mapSupabaseRowToStorefrontProduct(supabase, data as Record<string, unknown>, 0);
    } catch (err) {
